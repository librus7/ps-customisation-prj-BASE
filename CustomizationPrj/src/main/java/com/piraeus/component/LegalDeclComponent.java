...
public class LegalDeclComponent {
	...
	private static final Set<EventType> events_handled = Collections.unmodifiableSet(new TreeSet<EventType>() {{
		add(new EventType("ILC","ISI"));
		add(new EventType("IGT","IIG"));
		add(new EventType("ISB","IIS"));
	}});

	public static void onValidate(RouterCallbacks router, ValidationDetails validationDetails) {
		Loggers.method().enter(LOG);

		if (events_handled.contains(router.getEventType())) {
			...
			LOG.debug("Not Processing {}", router.getEventType());
		}
